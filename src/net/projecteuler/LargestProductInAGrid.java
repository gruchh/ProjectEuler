...
public class LargestProductInAGrid {
    ...
    public static void main(String[] args) {
        String declaredMatrix = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08\n" +
        ...
        List<List<Integer>> convertedListOfNumbers = new ArrayList<>();
        Scanner in = new Scanner(declaredMatrix);

        while (in.hasNextLine()) {
            convertedListOfNumbers.add(getValuesFromLineSeparatedBySpace(in.nextLine()));
        }
    ...
    private static String getBiggestWynikLiczbyObok(int stringLength, int matrixLength, List<List<Integer>> records) {
        int actualResultSide = 1;
        int actualResultVertical = 1;
        int biggestResultSide = 1;
        int biggestResultVertical = 1;
        for (int i = 0; i < matrixLength; i++) {
            for (int j = 0; j <= matrixLength - stringLength; j++) {
                actualResultSide = 1;
                actualResultVertical = 1;
                for (int k = 0; k < stringLength; k++) {
                    actualResultSide *= records.get(i).get(j + k);
                    actualResultVertical *= records.get(j + k).get(i);
                }

                if (actualResultSide > biggestResultSide) {
                    biggestResultSide = actualResultSide;
                }

                if (actualResultVertical > biggestResultVertical) {
                    biggestResultVertical = actualResultVertical;
                }
            }
        }
        return "Największa liczba poziomo: " + biggestResultSide + " Największa liczba pionowo: " + biggestResultVertical;
    }
    ...
    private static List<Integer> getValuesFromLineSeparatedBySpace(String givenCodeLine) {
        List<Integer> readValuesAsList = new ArrayList<Integer>();

        Scanner scannerCodeLine = new Scanner(givenCodeLine);
        scannerCodeLine.useDelimiter(" ");

        while (scannerCodeLine.hasNext()) {
            readValuesAsList.add(scannerCodeLine.nextInt());
        }

        return readValuesAsList;
    }
